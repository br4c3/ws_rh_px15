...
class PIDGuidanceNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('kp_final', 0.17)
        self.declare_parameter('ki_final', 0.0)
        self.declare_parameter('kd_final', 0.03)
        ...
        self.create_subscription(Bool,'/landing_triger', self.land_trigger_cb, qos_profile)
    ...
    def land_trigger_cb(self,msg:bool):
        if not msg.data:
            return
    ...
    def hold_command(self):
        ...
        self.publish_vehicle_command(
            ...
            param1=1.0,
            param2=float(PX4_CUSTOM_MAIN_MODE_AUTO),
            param3=float(PX4_CUSTOM_SUB_MODE_AUTO_LOITER)
        )
        self.is_hold = True
        self.is_offboard = False  # offboard 재진입 방지, loop() 조기 리턴
    ...
    def approach(self, dt:float):
        ...
        if not self.z_aligned_gate:
            ...
            if self.yaw_aligned_gate:
                v_yaw_b = 0.0
            else:
                v_yaw_b = self.pid_yaw.update(err_yaw, dt)

        # 최종 단계에서 yaw 정렬을 해야하면 xy속도는 0으로 고정
        elif not self.yaw_aligned_gate:
            vx_b = 0.0
            vy_b = 0.0
            vz_ned = 0.0
            v_yaw_b = self.pid_yaw.update(err_yaw, dt)
        ...
        if self.z_aligned_gate and self.yaw_aligned_gate and self.xy_aligned_gate:
            self.hold_command()
            return
    ...
    def gate_check(self,xy_err,alt):
        ...
        yaw_gate = abs(self.yaw_error_deg) <= self.yaw_tol_deg
        ...
        if yaw_gate: 
            if not self.yaw_aligned_gate:
                self.get_logger().info(f'yaw 정렬 완료 {self.yaw_error_deg}')
                self.pid_yaw.reset()
                self.yaw_aligned_gate = True

        elif abs(self.yaw_error_deg) >= 8.0:
            self.yaw_aligned_gate = False
        ...
        if self.z_aligned_gate and self.yaw_aligned_gate:
            if xy_gate:
                now = self.get_clock().now()
                ...
                if hold_time >= self.land_hold_time and not self.xy_aligned_gate:
                    self.get_logger().info('xy_gate 통과')
    ...
    def publish_setpoint(self, vel_vec, v_yaw_b: float = math.nan): 
        ...
        if math.isfinite(v_yaw_b):
            sp.yawspeed = math.radians(v_yaw_b)
        else:
            sp.yawspeed = 0.0
        ...
        self.get_logger().debug(
            f"Publishing velocity setpoints NED: "
            f"{vel_ned[0]:.3f}, {vel_ned[1]:.3f}, {vel_ned[2]:.3f}, "
            f"yaw_rate_deg={v_yaw_b:.1f}",  # yaw정렬 추가
