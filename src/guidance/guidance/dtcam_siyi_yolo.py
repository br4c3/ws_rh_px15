...
class VisionTrackerNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('qgc_video_width', 1980)
        self.declare_parameter('qgc_video_height', 1080)
        ...
        self.get_logger().info("✅ 모델 경로 등록 완료.")
    ...
    def detection_loop(self):
        ...
        if ret:
            ...
            self.process_yolo_and_center_detection(
                frame,
                self.center_point_pub,
                run_yolo=True,
            )
        
    def process_yolo_and_center_detection(self, frame, center_pub, run_yolo):
        ...
        if run_yolo and (self.current_model is not None):
            ...
            self.publish_target(results.boxes, center_pub, annotated_frame)
    ...
    def publish_target(self, boxes, center_pub, annotated_frame):
        if len(boxes) == 0:
            return
    ...
    def load_model_for_state(self, state: str):
        model_path = self.model_paths.get(state)

        if model_path is not None:
            self.get_logger().info(f"✅ Swapping to '{state}' model.")
            self.current_model = None
            self.current_model = YOLO(model_path)  
            self.get_logger().info(f"✅ '{state}' model loaded.")    
